...
# ╔═╡ ed8f202d-d83c-485e-9552-8ada8f1fcfb0
using Plots
...
# ╔═╡ a5a621a9-4682-4502-971d-cbc58728acda
n = 5

# ╔═╡ 277059e1-2440-449c-b27a-a3a1389f9cb2
rand(Int, n-1)

# ╔═╡ 2040aa8e-58f9-407c-9c18-f8834968578e
rand(Bool,n)
...
# ╔═╡ 5e0697f4-3baa-4cf4-816a-252c2b7a6603
aleatorioUniforme(10,16)
...
# ╔═╡ 008b07d5-1f1c-4b56-806c-b99fd892aa54
a = rand()

# ╔═╡ 0f299057-2956-4d6f-beee-c0a303605d3a
b = rand()
...
# ╔═╡ 1e136682-7570-4afc-8cb7-9379f6a3f482
UnPaso()
...
# Tu código va aquí :)
begin
	function Varios(n::Int)
		resultado = Array{Int}(undef,n)
		for i in 1:n
			if rand(Bool)
				resultado[i] = 1
			else
...
# ╔═╡ e5966c27-56e7-4e91-84d7-c5d61b8ce159
resultados = Varios(16)
...
begin 
function Caminata(n::Int)
	caminata = Array{Int}(undef,n+1)
	posicion = 0
	caminata[1] = posicion
	for i in 2:n+1
		if rand(Bool)
...
# ╔═╡ 59e6c834-874c-4075-9893-d7c8b89255cf
pasos = 20

# ╔═╡ 18d5b567-48cf-4ffd-9f85-6b3eab19bed1
resultado = Caminata(pasos)
...
# Tu código (comentado) va aquí :)
begin
	function grafica(resultado::Array{Int})
		numpasos= length(resultado)
		posiciones = resultado

	scatter(1:numpasos, posiciones, xlabel = "No. pasos",ylabel = "Posicion", legend = false)
	end
end


# ╔═╡ 45f0d0bb-fe85-47fb-b17c-866f5539344d
grafica(resultado)

# ╔═╡ 02b626b7-e639-4812-a734-d152531abe71
n2 = 15

# ╔═╡ f215d2b5-d661-4582-a358-3b8f52146e0c
caminata = Caminata(n2)

# ╔═╡ 67da2a42-1d14-453b-ad61-614ce471962a
grafica(caminata)
...
# Tu código (comentado) va aquí :)
function Grafica2(caminata::Array{Int})
	Numpasos = length(caminata)
	posiciones = caminata

plot(1:Numpasos, posiciones, xlabel ="No. pasos", ylabel= "posicion", legend = false )
end
...
begin
gr()
function animaCaminata(caminatal)                      # Definimos la función animaCaminata para una caminata
	n = length(caminatal)                              # definimos n como el tamaño de la caminata
anim = @animate for i in 1:n                           
	grafica(caminatal[1:i])                    # usamos la función graficaCaminata para una caminata
end
	return anim                                       
end
n3= 220                                                
caminatal = Caminata(n3)                      
anima = animaCaminata(caminatal)                       
gif(anima, "caminatal.gif" , fps =20)               
end
...
# Tu código (comentado) va aquí :)
begin
function caminataAleatoria2D(n)                                          # definimos la función 
	x = zeros(n+1)                                                       # definimos x como los ceros de n+1
	y = zeros(n+1)                                                       # definimos y como un arreglo de ceros para n+1
	for i in 2:n+1                                                       # para una i de 2 a n+1
		dx, dy = aleatorioUniforme(-1,1), aleatorioUniforme(-1,1)        # definimos dx y dy como una aleatorio uniforme de -1 a 1
	x[i] = x[i-1] +dx                                  
	y[i] = y[i-1] + dy                                
	end
	return [x, y]                                     
...
n4 = 1050                                                
caminatalea = caminataAleatoria2D(n4)                    
plot(caminatalea[1], caminatalea[2], xlabel ="Posición horizontal", ylabel= "Posición vertical", title= "Caminante aleatorio")             
end
...
# Tu código (comentado) va aquí :)
begin
function graficaCaminata2D(camino)                                        # definimos la función
x = [p[1] for p in camino]                                                # definimos x como p1 para p en e camino
	y = [p[2] for p in camino]                                            # definimos y como p2 para p en el camino
plot(x, y, linestyle =:dash, aspect_radio = 1, title= "Gráfica caminatas en 2D")  # graficamos x y y
end
N3 = 579                                                  
camino = caminataAleatoria2D(N3)                         
graficaCaminata2D(camino)                                 
end

# ╔═╡ f862b92b-bd8a-4bd0-a65d-c423877ec3ef
...
# Tu código (comentado) va aquí :)
begin
function graficaCaminata2D!(Caminata,g)                       # definimos la funcion
	x = [p[1] for p in Caminata]                              # definimos x
	y = [p[2] for p in Caminata]                              # definimos y
	plot!(g, x, y, linestyle=:dash, aspect_ratio=1)           # graficamos la funcion g y los puntos de x y y
    display(g)                     
end
	n5= 7689                                                  # para un n
g = plot(title= "Cinco caminatas aleatorias", xlabel= "Coordenada x", ylabel = "Coordenada y", legend = false, aspect_ratio=1)                               
for i = 1:5                                       
	Caminata = caminataAleatoria2D(n5)            
	graficaCaminata2D!(Caminata,g)               
end
	g
end
...
# Tu código (comentado) va aquí :)
function  caminataA2d(n::Int)
	posicion_h = zeros(n+1)
	posicion_v = zeros(n+1)
	for i in 2:n+1
		paso_h = aleatorioUniforme(-1,1)
		paso_v = aleatorioUniforme(-1,1)
		
		posicion_h[i] = posicion_h[i-1] + paso_h
		posicion_v[i] = posicion_v[i-1] + paso_v
	end
	
	return [posicion_h, posicion_v]
end

# ╔═╡ 9c8c316c-4cdd-4c9c-94e3-d6cac6675b67


# ╔═╡ 7123240a-884b-46e5-aa91-d11fbeed9fca
caminataA2d(12)
...
# ╔═╡ 483d1442-bcd8-425f-816d-a114c8c5f65d
begin
function caminataAleatoria3D(n)                         # definimos la funcion camnata aleatoria en 3dimensiones
	caminota = [[0.0, 0.0, 0.0]]                                               
	for i in 2:n                                      
		paso = [randn(), randn(), randn()]              
		nuevo_punto = caminota[i-1] + paso             
		push!(caminota, nuevo_punto)                  
	end
	return caminota                                     # regresamos a caminota
end
function graficaCaminata3D(caminota)                    # definimos la función grafica para la caminata en 3d
	x = [p[1] for p in caminota]                        # definimos x p1 para p en la caminata
	y = [p[2] for p in caminota]                        # definimos y p2 para p en la caminata
	z = [p[3] for p in caminota]                        # definimos z p3 para p en la caminata
scatter3d(x,y,z, markersize=2, markerstrokewidth =0, markeropacity= 0.5, legend= false )      # graficamos en x,y,z
	xlabel!("Coordenada x")
	ylabel!("Coordenada y")
	zlabel!("Coordenada z")
...
n7 = 10780                                         
caminota = caminataAleatoria3D(n)                   
graficaCaminata3D(caminota)                         
end

# ╔═╡ 48e666c6-3a29-4ebb-993f-06696a49b025
# Tu código (comentado) va aquí :)
function Grafica3(caminata)
	n= length(caminata)-1
	plot(0:n,caminata, xlabel = "paso", ylabel = "posicion")
end

# ╔═╡ 7137ddb0-4770-4b41-a33c-2320c0dc9a1f
Grafica3(caminata)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "afb300f1e410ce10aac5bd5380cdb59d5a3da28b"
