...
begin
	...
	function newton(f, f′, x0, n)
	    ...
	    for i in 1:n
	        x = x - f(x) / f′(x)  # Actualización de la aproximación usando el método de Newton
	    end
	
	    return x  # Devuelve la aproximación de la raíz después de n iteraciones
	end
	
end

# ╔═╡ 5f7ed3d0-6fd2-4ecc-8e4e-8e28a93ac337
newton(sin, cos, 1, 10)
...
# Tu código (comentado) va aquí :D
function newtonRecursiva(f, f′, x0, n)
    if n == 0 #Cuando no se ha dado ninguna iteración (n=0)
        return x0  # Entonces se devuelve la primera apróximación, la dada propiamente
	else #En otro caso, cuando hay n iteraciones
        x = x0 - f(x0) / f′(x0)  # Se actualiza la aproximación usando el método de Newton
        return newtonRecursiva(f, f′, x, n-1)  # Regresa la aproximación de la raíz hasta la iteración n-1
    end
end


# ╔═╡ 414a256b-df41-4306-a36f-9a3362543735
newtonRecursiva(sin, cos, 1, 10)
...
# Tu código (comentado) va aquí :D
	...
	begin
	function newtonDistancia(f, f′, x0, ε)
	    x_cero = x0  # Aproximación inicial
		x_uno = x_cero - f(x_cero) / f′(x_cero) #Primera aproximación
	    while abs(x_uno - x_cero) > ε
        x_cero = x_uno
        x_uno = x_uno - f(x_uno) / f′(x_uno)
    end
		return x_uno  # Devuelve la aproximación de la raíz
end
	end

# ╔═╡ 18e75773-17ac-48bb-b35c-fc967f6b45f1
newtonDistancia(sin, cos, 1, 1)
...
# Tu código (comentado) va aquí :D
derivaciónNumérica(f1, x_puntoderivable, h) = h * (f1(x_puntoderivable + (1/h)) - f1(x_puntoderivable))
	    

# ╔═╡ 69cdbcc0-d892-418c-872f-26e8bd1c5aae
derivaciónNumérica(sin, 1, 1000)
...
function newtonDN(f, x0, n, h)
        f′ = x -> derivaciónNumérica(f, x, h)  # Calcula la derivada numérica de f
        return newton(f, f′, x0, n)  # Aplica el método de Newton usando la derivada numérica
    end

# ╔═╡ 8fae80f0-f8e2-408f-a5db-b9d0317e8106
newtonDN(sin, 1, 10, 10000000000)
